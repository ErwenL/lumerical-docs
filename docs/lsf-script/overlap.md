...
Returns the overlap and power coupling between two modes calculated by the FDE solver or recorded by frequency monitors from an FDTD or varFDTD simulation, or field profiles recorded in rectilinear datasets.
...
out = overlap(mode2, mode1); | 
  ...
  * mode2, mode1: modes to calculate overlaps for, inputs can be 
    ...
    2. Names of frequency domain monitors in FDTD, string input, e.g., ‘m1’, ‘m2’
    3. Rectilinear datasets, see below for more information
  * out(1): the mode overlap
  * out(2): the mode power coupling

  
out = overlap(mode2, mode1, x, y,z); |  Mode alignment can be adjusted before overlap is calculated.

  * x offset
  * y offset
  * z offset

The offset is applied to the second mode listed, i.e. ‘mode1’ in this case. All inputs (FDE mode names, frequency monitors, rectilinear datasets) are valid also for the syntax with alignment adjustment.  
