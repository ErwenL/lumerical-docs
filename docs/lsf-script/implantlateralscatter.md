...
Calculates the 'lateral scatter' of the doping profile from an ion implant. The script command takes the semiconductor material name, dopant type, and ion energy as input arguments. 
...
The following script calculates the range, straggle, skewness, kurtosis, and lateral scatter for an ion implant in 'silicon' with 'boron' for an ion implant energy of 1 keV. The script then calculates the peak concentration for the 1D doping profile (without lateral scatter) for an ion dose of 2e13 /cm  2  and sets up an implant doping object in the CHARGE solver to model the corresponding doping profile. 
