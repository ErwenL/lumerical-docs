...
Fits any real-valued data, y = f(x), where x is a real valued argument, to the Pearson IV probability density function (PDF). The Pearson PDF is described by:

$$ \frac{1}{f(x)}\frac{df}{dx}=\frac{(x-\lambda)+a_{0}}{b_{0}+b_{1}(x-\lambda)+b_{2}(x-\lambda)^{2}} $$

It is categorized as type IV when the discriminant b0  +b1  x+b2  x  2  has no real roots. The Pearson IV PDF is typically defined in terms of the coefficients a0  ,b0  ,b1  and b2  that depend on the variance σ2  , skewness γ1  , and kurtosis β2  .
...
In the following example, an experimentally measured 1D doping profile is fitted with Pearson IV probability density function using the  fitpearson4pdf  script command. The example then plots the measured data and the fitted distribution on the same plot.
...
[ List of commands ](/hc/en-us/articles/360037228834) , [ normpdf ](/hc/en-us/articles/360034406454-normpdf) , [ pearson4pdf ](/hc/en-us/articles/360034926693-person4pdf) , [ fitnormpdf ](/hc/en-us/articles/360034926993-fitnormpdf)
