...
Original command: expand2
Translation date: 2026-02-04 22:49:48
-->

# expand2

返回 该 expansion coefficients 在 该 unconjugated form between 该 fields recorded at two arbitrary DFT monitors 或 saved 在 D-CARDs, 或 saved 在 two rectilinear datasets. The coefficients 在 该 unconjugated form 是 defined according 到:

$$ \begin{数组}{l}{一个=0.25^{*}\left(\frac{\int d \mathbf{S} \cdot \mathbf{E}_{1} \times \mathbf{H}_{2}}{N}+\frac{\int d \mathbf{S} \cdot \mathbf{E}_{2} \times \mathbf{H}_{1}}{N}\right)} \\\ {b=0.25^{*}\left(\frac{\int d \mathbf{S} \cdot \mathbf{E}_{1} \times \mathbf{H}_{2}}{N}-\frac{\int d \mathbf{S} \cdot \mathbf{E}_{2} \times \mathbf{H}_{1}}{N}\right)} \\\ {N=0.5 * \int d \mathbf{S}\cdot \mathbf{E}_{2} \times \mathbf{H}_{2}} \\\ {P=0.5^{*} \int d \mathbf{S} \cdot \mathbf{E}_{1} \times \mathbf{H}_{1}}\end{数组} $$

For more detail 在 如何 到 use 此 命令, definitions 在 该 参数 和 如何 到 interpret 该 results, please see [ Using Mode Expansion Monitors ](/hc/en-us/articles/360034902433-Using-Mode-Expansion-Monitors) .

**语法** |  **描述**  
---|---  
expand2(mode1,mode_ref,x,y,z); |  Outputs 该 expansion coefficients between 该 fields 的 two modes 在 该 unconjugated form
  ...
  * mode1, mode_ref: field information, 使用 \\(\mathbf{E}_{1},\mathbf{H}_{1}\\) contained 在 mode1, 和 \\(\mathbf{E}_{2},\mathbf{H}_{2}\\) contained 在 mode_ref. Can 为 在 one 的 该 following formats: 
    *       1. Frequency domain 监视器 names, 字符串 input, e.g. “monitor1”
      2. Mode D-CARDs containing field information, 字符串 input, e.g. “mode1”
      3. Rectilinear datasets, see below 用于 more info
...
The following 脚本 expands 该 fields 从 监视器 "R" onto 该 reference 监视器 "R_ref" 在 该 unconjugated form:
