...
Original command: implantrange
Translation date: 2026-02-04 22:50:00
-->

# implantrange

计算 该 'range' 的 该 1D doping profile 从 ion implant. The 脚本 命令 takes 该 semiconductor 材料 name, dopant 类型, 和 ion energy as input 参数. 

**语法** |  **描述**  
---|---  
out = implantrange("dopant", "semiconductor", E)  |  Provides 该 'range' 的 该 1D doping profile 从 ion implant 在 units 的 m. The "dopant" 参数 是 一个 字符串 providing 该 dopant 类型. The options 是 (i) "boron", (ii) "phosphorous", (iii) "antimony", 和 (iv) "arsenic". The "semiconductor" 参数 是 一个 字符串 providing 该 semiconductor 类型. The only available option 是 "silicon". The last 参数 (E) 是 该 ion energy 用于 该 implant 在 units 的 eV.   
