...
Original 命令: addconvectionbc
Translation date: 2026-02-04 23:30:06
-->
...
**语法** | **描述**
---|---
addconvectionbc; | 添加 一个 convection 边界条件 到 该 HEAT 或 CHARGE 求解器 (whichever 是 present 在 该 对象 tree)。 This 函数 does not 返回any 数据。
addconvectionbc("solver_name"); | 添加 一个 convection 边界条件 到 该 desired 求解器 defined 通过 该 参数 "solver_name"。 The options 是 "HEAT" 和 "CHARGE"。 This 函数 does not 返回any 数据。
