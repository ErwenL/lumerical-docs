...
Creates an a polygon (in 2D) or a planar solid (in 3D) using the finite-element geometric data stored in an unstructured dataset.
...
Run the script file [extract_2d.lsf](/hc/article_attachments/360045274574/extract_2d.lsf) with the CHARGE project file [geom2d.ldev](/hc/article_attachments/360046126993/geom2d.ldev) for a 2D example of this command. Here, we use the ID of the "CHARGE" solver region to single out any part of the structure with ID = 3, which would correspond to the semiconductor material in this example and then construct an object (polygon) in that shape.
