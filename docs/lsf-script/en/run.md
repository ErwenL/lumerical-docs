...
run("solver", "resource_type", "resource_name"); |  Launch the simulation using solver, resource_type, and with a specified resource_name:
...
run("FDTD", "GPU", "resource_name", CUDA_VISIBLE_DEVICE_values); |  Launch an FDTD GPU simulation with a specified resource_name and CUDA_VISIBLE_DEVICE_values:
